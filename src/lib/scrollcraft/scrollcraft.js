  ...
  function mount(root, opts) {
    ...
    Array.prototype.forEach.call(root.querySelectorAll('[data-sc-act]'), function (el) {
      ...
      var act = {
        ...
        video: null, seq: null, rail: null, railOver: 0
      };
    ...
    function layout() {
      ...
      acts.forEach(function (a) {
        ...
        a.top = r.top + scrollY;
        a.height = r.height;
        // scrollWidth is a layout read. Doing it inside read() put a forced
        // synchronous reflow on every scroll frame (the write to --sc-p just
        // above it invalidates layout first): cache it here instead, where it
        // is already batched with the other measurements and refreshed on
        // resize and font load.
        // web.dev/articles/avoid-large-complex-layouts-and-layout-thrashing
        if (a.rail) a.railOver = Math.max(a.rail.scrollWidth - vw, 0);
      });
      if (acts.length) {
        acts.forEach(function (a) {
          if (a.seq && a.seq.el) {
